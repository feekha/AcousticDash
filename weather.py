...
def get_weatherdata(df):
    '''
    Main function to get weather data from Brightsky API.
    '''
    locs, lats, lons = get_locs_lats_lons(df)
    ...
    for i in locs.index: 
        ...
        # request data
        url = 'https://api.brightsky.dev/weather?date={}&last_date={}&lat={}&lon={}'.format(first_date, last_date, float(lats[i]), float(lons[i]))
        ...
        all_weather_dict[locs[i]] = all_data_df
